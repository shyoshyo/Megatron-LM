...
class MegatronLiteEngine(BaseEngine):
    ...
    def save_checkpoint(
        ...
        **kwargs,
        ...
        save_hf_model = save_contents is None or "hf_model" in save_contents
    ...
    def _save_hf_checkpoint(self, local_path: str) -> None:
        ...
        # safetensors writes use mmap, which HDFS FUSE mounts do not support.
        # ``save_hf_weights`` honors MEGATRON_LITE_HF_CKPT_STAGE[_DIR] internally
        # to stage each shard on local disk before moving it into ``hf_local_path``.
